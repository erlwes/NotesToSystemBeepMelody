# Define the note-to-frequency lookup table
$noteFrequencies = @{
...
# Function to pre-calculate the melody
function Precalculate-Melody {
    param (
        [array]$melody,  # Array of note, duration, delay triplets
    ...
    $preCalculatedMelody = [System.Collections.Specialized.OrderedDictionary]::new()
    $i = 0 #Index/key

    foreach ($note in $melody) {
        $frequency = $noteFrequencies[$note[0]]
        $duration = switch ($note[1]) {
            "quarter" { $quarterNoteDuration }
            "eighth" { $eighthNoteDuration }
            "sixteenth" { $sixteenthNoteDuration }
            default { $quarterNoteDuration }  # Default to quarter note if unspecified
        }
        $delay = switch ($note[2]) {
            "quarter" { $quarterNoteDuration }
            "eighth" { $eighthNoteDuration }
            "sixteenth" { $sixteenthNoteDuration }
            default { $eighthNoteDuration }  # Default to eighth note delay if unspecified
        }
        $preCalculatedMelody.Add($i, @($frequency, $duration, $delay))
        $i++
    }    
    return $preCalculatedMelody
}

# Function to play pre-calculated melody
function Play-Melody {
    param (
        [System.Collections.Specialized.OrderedDictionary]$preCalculatedMelody
    )
    foreach ($key in $preCalculatedMelody.Keys) {
        $note = $preCalculatedMelody[$key]
        [System.Console]::Beep($note[0], $note[1])
        Start-Sleep -Milliseconds $note[2]
    }
}
